...
# print(Session.query(user_jm).count())
# tag =Session.query(user_jm).filter(user_jm.int_13!='NUll').count()
# print(tag)
tag =Session.query(user_jm).filter(user_jm.int_13==0).count()
tag1 =Session.query(user_jm).filter(user_jm.int_13.like('%jpg%')).count()

print(tag)
print(tag1)

users=Session.query(user_jm).filter(user_jm.id>(tag+tag1)).all()
...
for i in users:
    ...
    if len(resuslt)!=0:
        for j in resuslt:
            if 'jpg'in j.path:
                print(j.path)
                obj= Session.query(user_jm).filter(user_jm.id == i.id).first()
                obj.int_13 =j.path
                Session.commit()
                break
    else:
