...
# print(Session.query(user_wz).count())
tag =Session.query(user_wz).filter(user_wz.int_13==0).count()
tag1 =Session.query(user_wz).filter(user_wz.int_13.like('%jpg%')).count()

print(tag)
print(tag1)

users=Session.query(user_wz).filter(user_wz.id>(tag+tag1)).all()
