...
for i in l:
    if tag <=j :
        ...
        # print('-'*30)
        k  = s[0]
        to_data_wz(k.id_num, k.name, k.int_1,k.int_2 ,k.addr_1 ,k.data_b,k.int_3,k.addr_pjs, k.addr_2,k.int_4,k.ph_or_te,k.int_5,'NULL',k.int_6,k.int_7,k.int_8,k.int_9,k.int_10,'NULL')
        obj = Session.query(users).filter(users.id_num == i).first()
        obj.tag = '1'
        Session.add(obj)
        Session.commit()
